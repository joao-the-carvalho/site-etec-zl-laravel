...
<div class="space-y-5">
    <div>
        <label class="block text-sm text-gray-400 mb-1">Nome do Curso</label>
    ...
    <div>
        ...
        <textarea name="descricao" rows="4"
                  class="w-full bg-gray-800 border border-gray-700 text-white rounded-lg px-4 py-2 focus:border-red-500 focus:outline-none">{{ old('descricao', $curso->descricao ?? '') }}</textarea>
    </div>
        ...
        <div>
            ...
            <select name="turno" class="w-full bg-gray-800 border border-gray-700 text-white rounded-lg px-4 py-2 focus:border-red-500 focus:outline-none">
                @foreach(['Manhã','Tarde','Noite'] as $turno)
                    <option value="{{ $turno }}" {{ old('turno', $curso->turno ?? '') == $turno ? 'selected' : '' }}>{{ $turno }}</option>
                @endforeach
            </select>
        </div>
        <div>
            <label class="block text-sm text-gray-400 mb-1">Vagas</label>
            <input type="number" name="vagas" value="{{ old('vagas', $curso->vagas ?? '') }}" min="1"
                   class="w-full bg-gray-800 border border-gray-700 text-white rounded-lg px-4 py-2 focus:border-red-500 focus:outline-none">
        </div>
        <div>
            <label class="block text-sm text-gray-400 mb-1">Período</label>
            <input type="text" name="periodo" value="{{ old('periodo', $curso->periodo ?? '') }}" placeholder="ex: 3 anos"
                   class="w-full bg-gray-800 border border-gray-700 text-white rounded-lg px-4 py-2 focus:border-red-500 focus:outline-none">
        </div>
    </div>
    ...
    <div>
        ...
        @if(isset($curso) && $curso->imagem)
            <img src="{{ Storage::url($curso->imagem) }}" class="h-24 rounded mb-2 object-cover">
        @endif
